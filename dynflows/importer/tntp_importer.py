...
def natural_earth_projection(latInRad: float, lngInRad: float) -> Tuple[float, float]:
    l = (
        ...
        - 0.131979 * latInRad**2
        - 0.013791 * latInRad**4
        + 0.003971 * latInRad**10
        - 0.001529 * latInRad**12
    )
    d = latInRad * (
        1.007226
        + 0.015085 * latInRad**2
...
def import_network(edges_file_path: str) -> Network:
    net = pd.read_csv(edges_file_path, skiprows=8, sep="\t", skip_blank_lines=True)
    ...
    for _, e in net.iterrows():
        network.add_edge(
            int(e["init_node"]), int(e["term_node"]), e["free_flow_time"], e["capacity"]
        )
    return network
