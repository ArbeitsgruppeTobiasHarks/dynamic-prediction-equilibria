...
const CustomBox = (props) => <Box borderStyle='1px solid lightgray' borderRadius={20} {...props} />
...
const Presentation = () => (
    <Deck theme={theme} template={template}>
        <Slide backgroundColor='white'>
            ...
            <CustomBox position="absolute" top={250} width={1050} backgroundColor={theme.colors.tertiary} color={theme.colors.primary}>
        ...
        <CustomSlide section="I. The Flow Model">
            <Example>A dynamic prediction equilibrium.
                <UnorderedList>
                    <ListItem>We are given a single commodity with network inflow rate {Tex`u\equiv 2`} and
                        predictor {Tex`\hat q_{e}(\theta;\bar\theta; q) \coloneqq q_e(\bar\theta) + \partial_- q_e(\bar\theta)\cdot (\theta - \bar\theta)`}.</ListItem>
                    <ListItem>Only edge {Tex`ut`} can build a queue. For other edges {Tex`e\neq uv`} we have {Tex`\hat q_{i,e}(\theta;\bar\theta;q) = 0`}</ListItem>
                    <ListItem>Particles starting at {Tex`s`} need to decide between paths {Tex`sut`} and {Tex`svwt`}.</ListItem>
                    <ListItem>We have {Tex`\hat T_{svwt}(\theta;\bar\theta; q) = \theta + 4`} and {Tex`\hat T_{sut}(\theta;\bar\theta; q) = \theta + 2 + \hat q_{i,ut}(\theta + 1;\bar\theta; q)`}.</ListItem>
                </UnorderedList>
            </Example>
            <Example3Svg />
        </CustomSlide>
        ...
        <CustomSlide intro section="II. Existence of DPE">
            ...
            <Example>We are given
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}><div>
                    <UnorderedList>
                        <ListItem>a single commodity with network inflow rate {Tex`u \equiv 2`} with predictor</ListItem>
                        <ListItem>{Tex`
