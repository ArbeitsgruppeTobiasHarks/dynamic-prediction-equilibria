...
\todo{Explain the predict function} To speed up the calculation of shortest paths, we add a function \code{is_constant} which should return \code{True} if and only if the \code{predict} function of that predictor always returns constant functions.
...
As calculating shortest paths consumes a lot of time, we employ a cache in the attribute \code{_active_edges}.
Here, \code{_active_edges[c]} is a dictionary assigning active outgoing edges to nodes for a commodity \code{c}.
...
Once the user has successfully constructed such a builder and the attributes have been initialized, the user can call the function~\code{builder.build_until} as shown in~Algorithm~\ref{alg:extend-flow-builder} on it.
...
After saving these to the attribute \code{self._costs}, we clear the cache of the active edges and update \code{_route_time} and \code{_next_reroute_time}.
...
We first determine new constant inflow rates using the current dynamic costs functions in \code{self._costs}.
The maximum extension time is the minimum of the next reroute time and the next network inflow change time.
Then, we call \code{self._flow.extend} with these two arguments to extend the flow with the new inflow rates until either some edge outflow rate changes or the maximum extension time is reached.
This function call returns the set of edges whose outflow rate changed at time \code{self._flow.phi}.
Hence, we assign \code{self._handle_nodes} the set of target nodes of those edges. 
