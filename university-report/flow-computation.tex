...
For such a queue depletion of an edge $e$, we need to update the queue function \code{queues[e]} which will attain $0$ at its depletion time and should be extended by a $0$ slope.
...
For completeness, we describe the extension procedures for the three cases \textbf{I}-\textbf{III} in more detail.
Their corresponding implementations are displayed in Algorithms~\ref{alg:extend-case-i},~\ref{alg:extend-case-ii} and~\ref{alg:extend-case-iii}.
...
\end{minted}
\caption{Extension Procedure for Case~\textbf{I} in \code{class MultiComFlow}}
\label{alg:extend-case-i}
\end{algorithm}
...
def _extend_case_ii(self, e: Edge, new_inflow: List[float]):
  ...
  if e in self.depletions:
    self.depletions.pop(e)
    \end{minted}
\caption{Extension Procedure for Case~\textbf{II} in \code{class MultiComFlow}}
\label{alg:extend-case-ii}
\end{algorithm}
...
\end{minted}
\caption{Extension Procedure for Case~\textbf{III} in \code{class MultiComFlow}}
\label{alg:extend-case-iii}
\end{algorithm}
