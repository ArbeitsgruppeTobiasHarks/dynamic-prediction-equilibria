...
Finally, we have another priority queue \code{depletions} with events where an edge will supposedly deplete its queue.
That means, an edge \code{e} with prioritization key \code{depl_time} is contained in \code{depletions} if \code{queues[e](phi)} is positive and \code{queues[e]} reaches $0$ at time \code{depl_time}.
...
\end{itemize}
and initially empty priority queues \code{outflow_changes} and \code{depletions}.
...
Moreover, we want to choose $\alpha$ at least so small, that no edge outflow changes within $[\phi, \phi + \alpha)$.
...
This enables the mechanism to preserve flow conservation property~\ref{def:feasible-flow:flow-conservation} on intermediary nodes.

After determining the new inflow rate $(g_{i,e})_{i\in I}$ of an edge $e$ beginning at time $\phi$, we assign $\restrict{\infl_{i,e}}{[\phi,\infty)} \coloneqq g_{i,e}$.
...
\begin{proof}
    ...
    \begin{enumerate}[label=\textbf{Case \Roman*.}, wide=0.5em]
        ...
        \end{align*}
        And as property~\ref{def:feasible-flow:operate-at-capacity} already holds for $t\leq T_e(\theta)$ we conclude \(
            q_e^h(\theta - \transit_e) = q_e(\phi) - \capa_e \cdot \frac{q_e(\phi)}{\capa_e} = 0.
        \)
...
\begin{enumerate}[wide=0.5em]
    \item[\textbf{Case I and III.}] We denote the depletion time by $T_{\depl} \coloneqq \phi + q_e(\phi) / (\capa_e - g_e)$ and get \[
        q_e(\theta) = \begin{cases}
            q_e(\phi) - (\theta - \phi)\cdot (\capa_e - g_e), & \text{for $\theta \leq T_{\depl}$,}\\
            0, & \text{for $\theta \geq T_{\depl}$.}
        \end{cases}
    \]
    \item[\textbf{Case II.}] Here, no depletion occurs and for all $\theta \geq \phi$ we have \[
...
def _extend_case_ii(self, e: Edge, new_inflow: List[float]):
    ...
    for i in range(n):
        self.inflow[e][i].extend(phi, new_inflow[i])
    ...
    self.queues[e].extend_with_slope(phi, queue_slope)
    if self.depletions.contains(e):
...
def _extend_case_i(self, e: Edge):
    ...
    for i in range(n):
        ...
        self.outflow[e][i].extend(change_time, 0)
    ...
    elif self.depletions.contains(e):
        self.depletions.pop(e)
    \end{minted}
\end{algorithm}
...
def extend(
  ...
  for e in new_inflow.keys():
    ...
    if acc_in == 0:
      ...
      self._extend_case_ii(e, new_inflow)
    else:
      self._extend_case_iii(e, new_inflow)

  alpha = min(
    min(d[1] for d in self.depletions.values(),
        default=float('inf')) - self.phi
    self.outflow_changes.min_key() - self.phi,
    max_alpha
  )
  self.phi = phi + alpha

  while min(d[1] for d in self.depletions.values()) <= self.phi:
    e, d = min(self.depletions.items(), key=lambda (e,d): d[1])
    self.depletions.pop(e)
    self.queues[e].extend_with_slope(depl_time, 0.)
    if d[1] is not None:
      self.outflow_changes.add((e, d[1]), d[1])
      for i in range(n):
        self.outflow[e][i].extend(d[1], d[2][i])
