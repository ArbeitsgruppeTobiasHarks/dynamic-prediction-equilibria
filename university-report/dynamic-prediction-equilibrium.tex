...
\begin{definition}
    A predictor $\predq_{e}$  is called \emph{causal},
...
\begin{theorem}[{\cite[Theorem~10]{mainpaper}}]\label{thm:dpe-existence}
    For any network with a finite set of commodities, each associated with a locally integrable, bounded network inflow rate and causal, continuous, and FIFO-compatible predictors $\predq_{i,e}$ there exists a dynamic prediction flow with respect to $\predq$. 
\end{theorem}
...
\]
As this is a non-causal predictor, it is not possible to embed it into a simulation nor does it fulfill the constraints of Theorem~\ref{thm:dpe-existence}.
