...
from dynflows.core.predictors.constant_predictor import ConstantPredictor
from dynflows.core.predictors.linear_predictor import LinearPredictor
from dynflows.core.predictors.predictor_type import PredictorType
from dynflows.core.predictors.reg_linear_predictor import RegularizedLinearPredictor
from dynflows.core.predictors.zero_predictor import ZeroPredictor
from dynflows.eval.evaluate_network import eval_network_demand
from dynflows.importer.tntp_importer import (
    add_commodities,
    add_node_positions,
    import_network,
    natural_earth_projection,
)
from dynflows.utilities.get_tn_path import get_tn_path
from dynflows.visualization.to_json import to_visualization_json

from dpe_experiments.ml.build_test_flows import build_flows
from dpe_experiments.ml.generate_queues import (
    generate_queues_and_edge_loads,
    save_queues_and_edge_loads_for_flow,
)
from dpe_experiments.ml.SKFullNetworkModel import train_sk_full_net_model
from dpe_experiments.ml.SKNeighborhood import train_sk_neighborhood_model
from dpe_experiments.ml.TFNeighborhood import train_tf_neighborhood_model


def run_scenario(
    edges_tntp_path: str,
    trip_tntp_file_path: str,
    node_tntp_file_path: str,
    scenario_dir: str,
):
...
if __name__ == "__main__":
    ...
    def main():
        ...
        edges_tntp_path = os.path.join(
            tn_path, "Berlin-Friedrichshain/friedrichshain-center_net.tntp"
        )
        nodes_tntp_file_path = os.path.join(
            tn_path, "Berlin-Friedrichshain/friedrichshain-center_node.tntp"
        )
        trips_tntp_file_path = os.path.join(
            tn_path, "Berlin-Friedrichshain/friedrichshain-center_trips.tntp"
        )
        run_scenario(
            edges_tntp_path,
            trips_tntp_file_path,
            nodes_tntp_file_path,
            "./out/berlin-friedrichshain",
        )
