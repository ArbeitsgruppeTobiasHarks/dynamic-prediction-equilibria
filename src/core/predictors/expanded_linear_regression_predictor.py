...
class ExpandedLinearRegressionPredictor(Predictor):
        ...
        def predict(prediction_time: float, flow: DynamicFlow):
            ...
            for e_id, old_queue in enumerate(flow.queues):
                ...
                for i in range(past_timesteps + 1):
                    inputs[f"e[{-i}]"] = max(
                        0.0, old_queue(prediction_time - i * step_size)
                    )
                for k, e in enumerate(edges[e_id].node_from.outgoing_edges):
                    for i in range(11):
                        inputs[f"i{k}[{-i}]"] = max(
                            0.0, flow.queues[e.id](prediction_time - i * step_size)
                        )

                new_values = [inputs["e[0]"]] + [0.0] * future_timesteps
                for i in range(1, 11):
    ...
    @staticmethod
    def from_scikit_model(
        network: Network, model_path: str, past_timesteps: int, future_timesteps: int
    ):
        ...
        def predict(old_queues: List[PiecewiseLinear], phi: float):
            ...
            for e in network.graph.edges:
                ...
                if any(v != 0.0 for v in inputs):
                    ...
                    queues[e.id] = PiecewiseLinear([phi], [0.0], 0.0, 0.0)
            return queues
