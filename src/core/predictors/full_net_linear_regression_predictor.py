...
class FullNetLinearRegressionPredictor(Predictor):
    ...
    def predict(
        ...
        input_times = [prediction_time - t for t in range(-self._past_timesteps + 1, 1)]
        ...
        data = np.asarray(
            [
                ...
                [[load(t) for t in input_times] for load in edge_loads],
            ]
        )
        past_data = np.reshape(
            data, newshape=(len(self._network.graph.edges), 2 * self._past_timesteps)
        )
        ...
        for e_id, old_queue in enumerate(flow.queues):
            ...
            for i in range(1, len(new_values)):
                new_values[i] = max(
                    new_values[i],
                    new_values[i - 1]
                    - self._step_length * self._network.capacity[e_id],
                )
    ...
    def from_model(
        ...
        return FullNetLinearRegressionPredictor(
            lin_reg, test_mask, network, past_timesteps, future_timesteps, step_length
        )
