...
class PerEdgeLinearRegressionPredictor(Predictor):
    ...
    def predict(
        ...
        for e in edges:
            ...
            new_values = [0.0] * len(prediction)
            new_values[0] = prediction[0]
            for t in range(1, len(new_values)):
                new_values[t] = (
                    2 * prediction[t] - new_values[t - 1]
                    if self._average
                    else prediction[t]
                )
    ...
    def from_models(
        ...
        return PerEdgeLinearRegressionPredictor(
            models, past_timesteps, future_timesteps, network, average
        )
