...
class DiscreteFlowBuilder:
    network: Network
    predictors: Dict[PredictorType, Predictor]
    reroute_interval: float
    _active_routing: List[Dict[Node, Dict[Edge, float]]]
    _built: bool
    ...
    def __init__(
        ...
        self._built = False
        self._active_routing = [{} for _ in network.commodities]
    ...
    def build_flow(self) -> Generator[DynamicFlow, None, None]:
        ...
        while self._flow.phi < float("inf"):
            ...
            if self._flow.phi >= self._next_reroute_time:
                ...
                self._active_routing = [{} for _ in self.network.commodities]
    ...
    def _get_active_routing(self, i: int, s: Node) -> Dict[Edge, float]:
        if s in self._active_routing[i]:
            return self._active_routing[i][s]
            ...
            for v in com_nodes:
                ...
                for e in v.outgoing_edges:
                    ...
                    if w not in distances.keys():
                        continue
                    beta = 0.04
                    active_routing[e] = exp(-beta * distances[w]) / sum(exp(-beta*distances[other.node_to]) for other in v.outgoing_edges if other.node_to in distances)
                for j in self._commodity_ids_by_sink[commodity.sink]:
                    if (
                        self.network.commodities[j].predictor_type
                        == commodity.predictor_type
                    ):
                        self._active_routing[j][v] = active_routing
        return self._active_routing[i][s]
    ...
    def _determine_new_inflow(self) -> Dict[int, Dict[int, float]]:
        ...
        for v in self._handle_nodes:
            ...
            for i in used_commodities:
                ...
                active_routing = self._get_active_routing(i, v)
                for e in active_routing:
                    new_inflow[e.id][i] = inflow * active_routing[e]
        return new_inflow
