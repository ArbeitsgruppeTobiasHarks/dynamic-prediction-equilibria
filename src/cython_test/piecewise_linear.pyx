...
from src.cython_test.array_utils cimport elem_rank, elem_lrank, merge_sorted_many, merge_sorted

from cython.cimports.libc.math import fabs, fmax, fmin
...
cdef class PiecewiseLinear:
    ...
    def __init__(
        ...
        times: array.array,
        values: array.array,
        first_slope: cython.double,
        last_slope: cython.double,
    ...
    def _find_rnk_between(
        arr: array.array, x: cython.double, min_rnk: cython.Py_ssize_t, max_rnk: cython.Py_ssize_t
    ) -> cython.Py_ssize_t:
        low: cython.Py_ssize_t = min_rnk + 1
        high: cython.Py_ssize_t = max_rnk + 1
    ...
    def _recursive_eval_sorted_array(
        self,
        times: array.array,
        start_index: cython.Py_ssize_t,
        end_index: cython.Py_ssize_t,
        start_rnk: cython.Py_ssize_t,
        end_rnk: cython.Py_ssize_t,
        values: array.array,
    ):
        if end_index - start_index < 2:
            for i in range(start_index + 1, end_index):
                rnk = PiecewiseLinear._find_rnk_between(
                    self.times, times.data.as_doubles[i], start_rnk, end_rnk
                )
                values.data.as_doubles[i] = self._eval_with_rank(times.data.as_doubles[i], rnk)
        else:
            mid = (start_index + end_index) // 2
            mid_rnk = PiecewiseLinear._find_rnk_between(
                self.times, times.data.as_doubles[mid], start_rnk, end_rnk
            )
            values.data.as_doubles[mid] = self._eval_with_rank(times.data.as_doubles[mid], mid_rnk)
    ...
    @cython.ccall
    def eval_sorted_array(self, eval_times: array.array) -> array.array:
        eval_times_len: cython.Py_ssize_t = len(eval_times)
        if eval_times_len < 2:
            return array.array("d", [self.eval(time) for time in eval_times])
        values: array.array = array.clone(self.values, eval_times_len, zero=True)

        first_rnk: cython.Py_ssize_t = elem_rank(self.times, eval_times.data.as_doubles[0])
        values.data.as_doubles[0] = self._eval_with_rank(eval_times.data.as_doubles[0], first_rnk)
        last_rnk: cython.Py_ssize_t = PiecewiseLinear._find_rnk_between(
            self.times,
            eval_times.data.as_doubles[eval_times_len-1],
            first_rnk,
            len(self.times) - 1
        )
        values.data.as_doubles[eval_times_len-1] = self._eval_with_rank(eval_times.data.as_doubles[eval_times_len - 1], last_rnk)
        self._recursive_eval_sorted_array(
            eval_times, 0, eval_times_len - 1, first_rnk, last_rnk, values
        )
        return values
    ...
    def simplify(self) -> PiecewiseLinear:
        ...
        """
        new_times = array.array("d", (self.times[0],))
        new_values = array.array("d", (self.values[0], ))
    ...
    @cython.ccall
    def simplify2(self) -> PiecewiseLinear:
        """
        This removes unnecessary timesteps inplace.
        """
        simplified_until: cython.Py_ssize_t = 0
        gradient_starting_at_su: cython.double = self.gradient(simplified_until)

        original_len: cython.Py_ssize_t = len(self.times)
        i: cython.Py_ssize_t
        for i in range(0, original_len - 1):
            # Add i+1, if it's necessary.
            gradient_starting_from_iplus1: cython.double = self.gradient(i + 1)

            if fabs(gradient_starting_at_su - gradient_starting_at_su) >= 1000 * eps:
                simplified_until += 1
                self.times.data.as_doubles[simplified_until] = self.times.data.as_doubles[i + 1]
                self.values.data.as_doubles[simplified_until] = self.values.data.as_doubles[i + 1]
                gradient_starting_at_su = gradient_starting_from_iplus1
                  
        array.resize(self.times, simplified_until + 1)
        array.resize(self.values, simplified_until + 1)
        return self

    @cython.ccall
    def _eval_with_rank(self, at: cython.double, rnk: cython.Py_ssize_t) -> cython.double:
        assert self.domain[0] <= at <= self.domain[1], f"Function not defined at {at}."
        ...
        size: cython.Py_ssize_t = len(self.values)

        if rnk == -1:
            first_grad = self.gradient(rnk)
            if at == neg_inf and first_grad == 0:
                return self.values.data.as_doubles[0]
            else:
                return self.values.data.as_doubles[0] + (at - self.times.data.as_doubles[0]) * first_grad
        elif rnk == size - 1:
            last_grad = self.gradient(rnk)
            if at == inf and last_grad == 0:
                return self.values.data.as_doubles[size - 1]
            else:
                return self.values.data.as_doubles[size - 1] + (at - self.times.data.as_doubles[size - 1]) * last_grad
        return self.values.data.as_doubles[rnk] + (at - self.times.data.as_doubles[rnk]) * self.gradient(rnk)

    @cython.ccall
    def gradient(self, i: cython.Py_ssize_t) -> cython.double:
        """
        ...
        return (self.values.data.as_doubles[i + 1] - self.values.data.as_doubles[i]) / (
            self.times.data.as_doubles[i + 1] - self.times.data.as_doubles[i]
        )
    ...
    def plus(self, other: PiecewiseLinear) -> PiecewiseLinear:
        ...
        """
        cdef (double, double) new_domain = (
            fmax(self.domain[0], other.domain[0]),
            fmin(self.domain[1], other.domain[1]),
        )
        assert (
            new_domain[0] < new_domain[1]
        ), "Intersection of function domains is empty."

        times: array.array = merge_sorted(self.times, other.times)
        min_time_idx: cython.Py_ssize_t = 0

        if times.data.as_doubles[0] < new_domain[0]:
            # cut times below new_domain[0]
            rnk: cython.Py_ssize_t = elem_rank(times, new_domain[0])  # => rnk >= 0
            if times.data.as_doubles[rnk + 1] == new_domain[0]:
                min_time_idx = rnk + 1
                times = times[rnk + 1 :]
            else:
        ...
        len_times: cython.Py_ssize_t = len(times)
        for i in range(len_times):
            values.data.as_doubles[i] = self.eval(times.data.as_doubles[i]) + other.eval(times.data.as_doubles[i])
    ...
    def minimum(self, otherf: PiecewiseLinear) -> PiecewiseLinear:
        ...
        values = array.array("d", (min(self(t), otherf(t)) for t in times))
...
identity = PiecewiseLinear(array.array("d", [0.0]), array.array("d", [0.0]), 1.0, 1.0)
zero = PiecewiseLinear(array.array("d", [0.0]), array.array("d", [0.0]), 0.0, 0.0)
