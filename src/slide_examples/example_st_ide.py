...
def build_example_nash():
    ...
    loader = NetworkLoader(network, [
        ([e1, e2, e3], RightConstant(times, values1, (0, float('inf')))),
        ([e1, e4, e5], RightConstant(times, values2, (0, float('inf'))))
    ])
    ...
    to_visualization_json("./exampleStIde.vis.json", flow, network, color_by_comm_idx)
