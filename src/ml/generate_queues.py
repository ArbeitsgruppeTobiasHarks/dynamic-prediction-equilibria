...
def generate_queues(
    ...
    files = [
        file
        for file in os.listdir(flows_folder)
    ...
    for flow_path in files:
        ...
        queues = np.asarray([[queue(time) for time in times] for queue in flow.queues])
        np.savetxt(queue_path, queues)


def save_queues_and_edge_loads_for_flow(
    out_path: str,
    past_timesteps: int,
    horizon: float,
    ...
    if (
        abs(
            reroute_intervals_in_prediction_interval
            - prediction_interval / reroute_interval
        )
        > 1e-14
    ):
    ...
    queue_samples = np.asarray(
        [queue.eval_sorted_array(times) for queue in flow.queues]
    )
...
def generate_queues_and_edge_loads(
    past_timesteps: int,
    flows_dir: str,
    out_dir: str,
    horizon: float,
    ...
    for flow_filename in files:
        ...
        out_path = os.path.join(out_dir, f"{flow_id}-queues-and-edge-loads.npy")
...
def generate_training_data_with_edge_loads(
    past_timesteps: int,
    ...
    for flow_path in files:
        flow_id = flow_path[: len(flow_path) - len(".flow.pickle")]
        out_path = os.path.join(training_data_dir, f"{flow_id}-training-data.npy")
        if os.path.exists(out_path):
            print(f"Training data for flow#{flow_id} already exists. Skipping...")
            continue
        ...
        for prediction_time in prediction_times:
            past_times = [
                prediction_time + i * prediction_interval
...
def expanded_queues_from_flows(
    network_path: str,
    past_timesteps: int,
    step_length: float,
    future_timesteps: int,
    ...
    df = pd.DataFrame(
        samples,
        columns=[f"i{e}[{t}]" for e in range(5) for t in range(-past_timesteps, 1)]
...
def expanded_queues_from_flows_per_edge(
    network_path: str,
    past_timesteps: int,
    step_length: float,
    future_timesteps: int,
            ...
            for phi in sample_times:
                ...
                for ie in e.node_from.incoming_edges:
                    for t in range(-past_timesteps + 1, 1):
                        d[f"{ie.id}[{t}]"] = (
                            flow.queues[ie.id].integrate(
                                phi - step_length * (t + 1), phi - step_length * t
                            )
                            if average
                            else flow.queues[ie.id](phi - step_length * t)
                        )
                for oe in e.node_to.outgoing_edges:
                    for t in range(-past_timesteps + 1, 1):
                        d[f"{oe.id}[{t}]"] = (
                            flow.queues[oe.id].integrate(
                                phi - step_length * (t + 1), phi - step_length * t
                            )
                            if average
                            else flow.queues[oe.id](phi - step_length * t)
                        )
                for t in range(-past_timesteps + 1, future_timesteps + 1):
                    d[f"{e.id}[{t}]"] = (
                        flow.queues[e.id].integrate(
                            phi - step_length * (t + 1), phi - step_length * t
                        )
                        if average
                        else flow.queues[e.id](phi - step_length * t)
                    )
                samples.append(d)
