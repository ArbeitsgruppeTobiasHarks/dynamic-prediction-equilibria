...
class QueueDataset(Dataset):
    ...
    def __init__(
        ...
        if self._in_memory:
            ...
            for file in self._queue_files:
                ...
                self.samples_per_flow = (
                    queues.shape[1] - past_timesteps - future_timesteps
                )
            print("Done reading dataset.")
            self.test_mask = torch.tensor(
                [max_queue > 0 for max_queue in max_queues]
            ).to(torch_mode)
            np.savetxt("../../out/mask.txt", self.test_mask)
        else:
            np_mask = np.genfromtxt("../../out/mask.txt")
            self.test_mask = torch.tensor([max_queue > 0 for max_queue in np_mask]).to(
                torch_mode
            )
    ...
    def __getitem__(self, index) -> T_co:
        ...
        stride = (
            queues.shape[1] - self._past_timesteps - self._future_timesteps
        ) // self.samples_per_flow
        ...
        return (
            past_queues.to(self._torch_mode)[self.test_mask],
            future_queues.to(self._torch_mode)[self.test_mask],
        )

    def __len__(self):
        return (
            len(self._queues) if self._in_memory else len(self._queue_files)
        ) * self.samples_per_flow
