...
class QueueAndEdgeLoadDataset:
    ...
    def __init__(
        ...
        self._times = [
            i * reroute_interval
            for i in range(
                -(past_timesteps - 1) * self._reroute_intervals_in_prediction_interval,
        ...
        for file in self._data_files:
            ...
            self.samples_per_flow = data.shape[
                2
    ...
    def __getitem__(self, index):
        ...
        input_mask = (
            self.input_mask
            if self._additional_input_mask is None
            else self.input_mask * self._additional_input_mask
        )
        output_mask = (
            self.output_mask
            if self._additional_output_mask is None
            else self.output_mask * self._additional_output_mask
        )
        ...
        future_queues = data[
            0, output_mask, phi_ind + stride : last_ind + 1 : stride
        ].flatten()
    ...
    def load_mask(folder_path: str) -> Tuple[np.ndarray, np.ndarray]:
        input_mask = np.array(
            [v > 0 for v in np.loadtxt(os.path.join(folder_path, "../input-mask.txt"))]
        )
        output_mask = np.array(
            [v > 0 for v in np.loadtxt(os.path.join(folder_path, "../output-mask.txt"))]
        )
