...
class Model(nn.Module):
    def __init__(
        self, past_queue_steps: int, future_queue_steps: int, conv_layers: int
    ):
...
if __name__ == "__main__":
    ...
    new_edges = [
        (e1.id, e2.id) for e1 in network.graph.edges for e2 in e1.node_to.outgoing_edges
    ]
    ...
    queue_dataset = QueueDataset(
        queue_folder_path, past_timesteps, future_timesteps, network, False
    )

    mask = queue_dataset.test_mask.cpu().numpy()
    np.savetxt(
        "/home/michael/Nextcloud/Universität/2021-SS/softwareproject/data/generated_queues/5,5/mask.txt",
        mask,
        fmt="%d",
    )
    ...
    capacity = (
        torch.from_numpy(network.capacity).float().to("cuda")[queue_dataset.test_mask]
    )
    travel_time = (
        torch.from_numpy(network.travel_time)
        .float()
        .to("cuda")[queue_dataset.test_mask]
    )
    ...
    data_loader = DataLoader(
        queue_dataset, batch_size=1, shuffle=True, collate_fn=collate
    )
