...
If we use try to estimate upcoming travel times using projected queue values $\hat{q}_e = q_e(\theta) + \frac{q_e(\theta) - q_e(\theta - \delta)}{\delta} \cdot w$, fluctuation decays over time, see Figure \ref{fig:fluctuations_proj}. Decay seems to be present only if $w > \Delta \theta$.	
