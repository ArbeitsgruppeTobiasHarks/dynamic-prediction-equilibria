...
class FullNetLinearRegressionPredictor(Predictor):
    ...
    def __init__(self, lin_reg: LinearRegression, test_mask: np.ndarray, network: Network, past_timesteps: int, future_timesteps: int):
    ...
    def predict(self, prediction_time: float, flow: DynamicFlow) -> List[PiecewiseLinear]:
        ...
        future_queues = self._lin_reg.predict(
            [[phi] + past_data[self._test_mask].flatten()])[0]
        future_queues = np.maximum(future_queues, np.zeros_like(future_queues))
    ...
    @staticmethod
    def from_model(network: Network, model_path: str, test_mask, past_timesteps: int, future_timesteps: int):
        with open(model_path, "rb") as file:
            lin_reg: LinearRegression = pickle.load(file)
        return FullNetLinearRegressionPredictor(lin_reg, test_mask, network, past_timesteps, future_timesteps)
