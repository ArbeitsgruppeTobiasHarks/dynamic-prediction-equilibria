...
class NashFlowBuilder:
    ...
    def _get_initial_path_dist(self) -> List[List[PathInflow]]:
        ...
        for i, commodity in enumerate(self.network.commodities):
            ...
            sink = commodity.sink
            dist = reverse_dijkstra(sink, costs, set(
                self.network.graph.nodes.values()))
            ...
            while v != sink:
                edge = min(
                    v.outgoing_edges, key=lambda e: dist[e.node_to] + costs[e.id] - dist[v])
                path.append(edge)
                v = edge.node_to
    ...
    def compute_shortest_paths_over_time(self, earliest_arrival_fcts: Dict[Node, PiecewiseLinear], edge_costs: List[PiecewiseLinear], source: Node, sink: Node) -> PathsOverTime:
        '''
        ...
        while dep_time < float('inf'):
            ...
            for labelled_edge in reversed(labelled_path):
                last_enter_time_st_rest_path_active = edge_exit_times[labelled_edge.edge].max_t_below(rest_of_path_active_until)
    ...
    def iterate(self, iteration: int, prev_flow: DynamicFlow, prev_path_dist: List[List[PathInflow]]) -> Tuple[DynamicFlow, List[List[PathInflow]]]:
        ...
        for i, _ in enumerate(self.network.commodities):
            ...
            for path_idx, (path, shortest_paths_over_time, delay) in enumerate(result):
                ...
                new_path_inflow = (new_path_inflow + prev_inflow - fraction_of_delayed_inflow).simplify()
                new_path_inflows.set(path, new_path_inflow)
                
                assert fraction_of_delayed_inflow.times[0] == fraction_of_delayed_inflow.domain[0]
                shortest_path_idx = 0
                for j in range(len(fraction_of_delayed_inflow.times)):
                    if fraction_of_delayed_inflow.values[j] > 0:
                        ...
                        while (j+1 < len(fraction_of_delayed_inflow.times) and already_shifted_until < fraction_of_delayed_inflow.times[j+1]) or already_shifted_until == float('inf'):                            
                            # Find shortest path at time delayed_inflow.times[j]
                            while already_shifted_until >= shortest_paths_over_time.times[shortest_path_idx]:
                                shortest_path_idx += 1

                            shortest_path = shortest_paths_over_time.paths[shortest_path_idx]
                            shift_until = min(
                                shortest_paths_over_time.times[shortest_path_idx],
                                fraction_of_delayed_inflow.times[j+1] if j+1 < len(fraction_of_delayed_inflow.times) else float('inf')
                            )
                            ...
                            shift_times = []
                            shift_values = []
                            if fraction_of_delayed_inflow.domain[0] < fraction_of_delayed_inflow.times[j]:
                            ...
                            sh_path_inflow = sh_path_inflow + RightConstant(shift_times, shift_values, fraction_of_delayed_inflow.domain)
        ...
        builder = NetworkLoader(
            self.network,
            [path_inflow for path_inflows in new_path_dist for path_inflow in path_inflows]
        ).build_flow()
        flow = next(builder)
