...
def train_model(queues_folder, past_timesteps, future_timesteps, network: Network, out_folder):
    os.makedirs(out_folder, exist_ok=True)

    queue_dataset = QueueAndEdgeLoadDataset(queues_folder, past_timesteps, future_timesteps, network)

    X, Y = zip(*queue_dataset)
    X, Y = np.array(X), np.array(Y)

    hidden_layer_sizes=(8*X.shape[1], 4*X.shape[1], 2*X.shape[1], X.shape[1], X.shape[1]//2)
